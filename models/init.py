...
def LSUVinit(model, data, needed_std=1.0, std_tol=0.1, max_attempts=10, do_orthonorm=True, cuda=False):
    ...
    if do_orthonorm:
        ...
        logging.debug('Orthonorm done')
        if cuda:
            model = model.cuda()
    for layer_idx in range(gg['total_fc_conv_layers']):
        logging.debug(layer_idx)
        model.apply(add_current_hook)
        out = model(data[0])
        current_std = gg['act_dict'].std()
        logging.debug('std at layer: {}  = {}'.format(layer_idx, current_std))
        ...
        while (np.abs(current_std - needed_std) > std_tol):
            ...
            out = model(data[0])
            current_std = gg['act_dict'].std()
            logging.debug('std at layer: {} =  {}, mean = {}'.format(layer_idx, current_std, gg['act_dict'].mean()))
            attempts += 1
            if attempts > max_attempts:
                logging.debug('Cannot converge in {} iterations'.format(max_attempts))
                break
        if gg['hook'] is not None:
            gg['hook'].remove()
        gg['done_counter'] += 1
        ...
        gg['hook_position'] = 0
        gg['hook'] = None
        logging.debug('finish at layer: {}'.format(layer_idx))
    logging.debug('LSUV init done!')
    if not cuda:
        model = model.cpu()
    model.train()
    return model
